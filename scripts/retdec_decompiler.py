...
def check_arguments () :
    ...
    if str(MODE) == 'll':
        ...
        if if str(CONFIG_DB) == str():
            not str(NO_CONFIG) != '':
            subprocess.call(['print_error_and_die','Option --config or --no-config must be specified in mode '+str(MODE)],shell=True)
    elif str(MODE) == 'raw':
        ...
        if not str(RAW_ENTRY_POINT) != '':
            subprocess.call(['print_error_and_die','Option --raw-entry-point must be used with mode '+str(MODE)],shell=True)
        if not str(RAW_SECTION_VMA) != '':
            subprocess.call(['print_error_and_die','Option --raw-section-vma must be used with mode '+str(MODE)],shell=True)
    ...
    if (str(OUT) == '' ):
        ...
        if (str(IN##*.) == 'll' ):
            # Suffix .ll
            Make('OUT').setValue(str(IN%.ll)+'.'+str(HLL))
        elif (str(IN##*.) == 'exe' ):
            # Suffix .exe
            Make('OUT').setValue(str(IN%.exe)+'.'+str(HLL))
        elif (str(IN##*.) == 'elf' ):
            # Suffix .elf
            Make('OUT').setValue(str(IN%.elf)+'.'+str(HLL))
        elif (str(IN##*.) == 'ihex' ):
            # Suffix .ihex
            Make('OUT').setValue(str(IN%.ihex)+'.'+str(HLL))
        elif (str(IN##*.) == 'macho' ):
            # Suffix .macho
            Make('OUT').setValue(str(IN%.macho)+'.'+str(HLL))
        else:
            Make('OUT').setValue(str(IN)+str(PICKED_FILE)+'.'+str(HLL))
    ...
    if (str(IN) == str(OUT) ):
        Make('OUT').setValue(str(IN%.*)+'.out.'+str(HLL))
    # Convert to absolute paths.
    IN=Bash2Py(os.popen('get_realpath \''+str(IN)+'\'').read().rstrip('\n'))
    OUT=Bash2Py(os.popen('get_realpath \''+str(OUT)+'\'').read().rstrip('\n'))
    if (os.path.exists(str(PDB_FILE)) ):
        Make('PDB_FILE').setValue(os.popen('get_realpath \''+str(PDB_FILE)+'\'').read().rstrip('\n'))
    # Check that selected ranges are valid.
    if (str(SELECTED_RANGES) != '' ):
        for Make('r') in Array(SELECTED_RANGES[@] ]):
            # Check if valid range.
            if (not subprocess.call(['is_range',str(r)],shell=True) ):
                subprocess.call(['print_error_and_die','Range ''+str(r)+'' in option --select-ranges is not a valid decimal (e.g. 123-456) or hexadecimal (e.g. 0x123-0xabc) range.'],shell=True)
            # Check if first <= last.
            Make('IFS').setValue('-')
            # parser line into array
            if ((vs[0] ] > vs[1] ]) ):
                subprocess.call(['print_error_and_die','Range ''+str(r)+'' in option --select-ranges is not a valid range: second address must be greater or equal than the first one.'],shell=True)
...
def print_warning_if_decompiling_bytecode () :
    ...
    Make('BYTECODE').setValue(os.popen('\''+str(CONFIGTOOL)+'\' \''+str(CONFIG)+'\' --read --bytecode').read().rstrip('\n'))
    if (str(BYTECODE) != str() ):
        subprocess.call(['print_warning','Detected '+str(BYTECODE)+' bytecode, which cannot be decompiled by our machine-code decompiler. The decompilation result may be inaccurate.'],shell=True)
...
def check_whether_decompilation_should_be_forcefully_stopped (_p1) :
    ...
    if (str(STOP_AFTER) == str(_p1) ):
        if str(GENERATE_LOG) != '':
            subprocess.call(['generate_log'],shell=True)
        subprocess.call(['cleanup'],shell=True)
        print()
        print('#### Forced stop due to '--stop-after '+str(STOP_AFTER)+''...')
        exit(0)
...
def get_tool_rc (_p1,_p2) :
    ...
    if (re.search(str(SIGNAL_REGEX),str(OUTPUT)) ):
        Make('SIGNAL_NUM').setValue(BASH_REMATCH[1] ])
        Make('RC').setValue((SIGNAL_NUM + 128))
    else:
        Make('RC').setValue(ORIGINAL_RC)
    ...
    if (str(RC) == '134' ):
        if (re.search('std::bad_alloc',str(OUTPUT)) ):
...
def get_tool_runtime (_p1) :
    ...
    RUNTIME_F=Bash2Py(os.popen('echo '+str(USER_TIME_F)+' + '+str(SYSTEM_TIME_F)+' | bc').read().rstrip('\n'))
...
def timed_kill (_p1) :
    ...
    PROCESS_NAME=Bash2Py(os.popen('ps -p '+str(PID)+' -o comm --no-heading').read().rstrip('\n'))
    if (str(PROCESS_NAME) == 'time' ):
        ...
        # usage etc.).
        Make('PID').setValue(os.popen('ps --ppid '+str(PID)+' -o pid --no-heading | head -n1').read().rstrip('\n'))
    if (str(TIMEOUT) == '' ):
        Make('TIMEOUT').setValue(300)
    timeout=Bash2Py(TIMEOUT)
    
    Make('t').setValue(timeout)
    while ((t > 0)):
        subprocess.call(['sleep','1'],shell=True)
        if not subprocess.call('kill' + ' ' + '-0' + ' ' + str(PID),shell=True,stdout=file(str(DEV_NULL),'wb'),stderr=file(str(DEV_NULL),'wb'))
        :
            exit(0)
        Make('t').setValue((t - 1))
    _rc0 = subprocess.call('kill_tree' + ' ' + str(PID) + ' ' + 'SIGKILL',shell=True,stdout=file(str(DEV_NULL),'wb'),stderr=file(str(DEV_NULL),'wb'))
...
def kill_tree (_p1) :
    ...
    for Make('_child') in Array(os.popen('ps -o pid --no-headers --ppid \''+Expand.underbar()+'pid\'').read().rstrip('\n')):
...
def generate_log () :
    ...
    Make('LOG_FILE').setValue(str(OUT)+'.decompilation.log')
    LOG_DECOMPILATION_END_DATE=Bash2Py(os.popen('date +%s').read().rstrip('\n'))
    LOG_FILEINFO_OUTPUT=Bash2Py(os.popen('json_escape \''+str(LOG_FILEINFO_OUTPUT)+'\'').read().rstrip('\n'))
    LOG_UNPACKER_OUTPUT=Bash2Py(os.popen('json_escape \''+str(LOG_UNPACKER_OUTPUT)+'\'').read().rstrip('\n'))
    LOG_BIN2LLVMIR_OUTPUT=Bash2Py(os.popen('remove_colors \''+str(LOG_BIN2LLVMIR_OUTPUT)+'\'').read().rstrip('\n'))
    LOG_BIN2LLVMIR_OUTPUT=Bash2Py(os.popen('json_escape \''+str(LOG_BIN2LLVMIR_OUTPUT)+'\'').read().rstrip('\n'))
    LOG_LLVMIR2HLL_OUTPUT=Bash2Py(os.popen('remove_colors \''+str(LOG_LLVMIR2HLL_OUTPUT)+'\'').read().rstrip('\n'))
    LOG_LLVMIR2HLL_OUTPUT=Bash2Py(os.popen('json_escape \''+str(LOG_LLVMIR2HLL_OUTPUT)+'\'').read().rstrip('\n'))
    log_structure=Bash2Py('{\n\t\'input_file\' : \'%s\',\n\t\'pdb_file\' : \'%s\',\n\t\'start_date\' : \'%s\',\n\t\'end_date\' : \'%s\',\n\t\'mode\' : \'%s\',\n\t\'arch\' : \'%s\',\n\t\'format\' : \'%s\',\n\t\'fileinfo_rc\' : \'%s\',\n\t\'unpacker_rc\' : \'%s\',\n\t\'bin2llvmir_rc\' : \'%s\',\n\t\'llvmir2hll_rc\' : \'%s\',\n\t\'fileinfo_output\' : \'%s\',\n\t\'unpacker_output\' : \'%s\',\n\t\'bin2llvmir_output\' : \'%s\',\n\t\'llvmir2hll_output\' : \'%s\',\n\t\'fileinfo_runtime\' : \'%s\',\n\t\'bin2llvmir_runtime\' : \'%s\',\n\t\'llvmir2hll_runtime\' : \'%s\',\n\t\'fileinfo_memory\' : \'%s\',\n\t\'bin2llvmir_memory\' : \'%s\',\n\t\'llvmir2hll_memory\' : \'%s\'\n}\n')
    print( str(log_structure) % (str(IN), str(PDB_FILE), str(LOG_DECOMPILATION_START_DATE), str(LOG_DECOMPILATION_END_DATE), str(MODE), str(ARCH), str(FORMAT), str(LOG_FILEINFO_RC), str(LOG_UNPACKER_RC), str(LOG_BIN2LLVMIR_RC), str(LOG_LLVMIR2HLL_RC), str(LOG_FILEINFO_OUTPUT), str(LOG_UNPACKER_OUTPUT), str(LOG_BIN2LLVMIR_OUTPUT), str(LOG_LLVMIR2HLL_OUTPUT), str(LOG_FILEINFO_RUNTIME), str(LOG_BIN2LLVMIR_RUNTIME), str(LOG_LLVMIR2HLL_RUNTIME), str(LOG_FILEINFO_MEMORY), str(LOG_BIN2LLVMIR_MEMORY), str(LOG_LLVMIR2HLL_MEMORY)) )
    

# Check script arguments.
PARSED_OPTIONS=Bash2Py(os.popen('getopt -o \''+str(GETOPT_SHORTOPT)+'\' -l \''+str(GETOPT_LONGOPT)+'\' -n \''+str(SCRIPT_NAME)+'\' -- \''+Str(Expand.at())+'\'').read().rstrip('\n'))
# Bad arguments.
if _rc0 != 0:
    subprocess.call(['print_error_and_die','Getopt - parsing parameters fail'],shell=True)
eval('set','--',str(PARSED_OPTIONS))
while (True):
    ...
    if ( str(sys.argv[1]) == '-a' or str(sys.argv[1]) == '--arch'):
        ...
        if (not os.access(str(PDB_FILE),R_OK) ):
            subprocess.call(['print_error_and_die','The input PDB file ''+str(PDB_FILE)+'' does not exist or is not readable'],shell=True)
        subprocess.call(['shift','2'],shell=True)
        ...
        if (not os.access(str(CONFIG_DB),R_OK) ):
            subprocess.call(['print_error_and_die','The input JSON configuration file ''+str(CONFIG_DB)+'' does not exist or is not readable'],shell=True)
        subprocess.call(['shift','2'],shell=True)
        ...
        if (not re.search('^(fileinfo|unpacker|bin2llvmir|llvmir2hll)'+'$',str(STOP_AFTER)) ):
            subprocess.call(['print_error_and_die','Unsupported tool ''+str(STOP_AFTER)+'' for --stop-after'],shell=True)
        subprocess.call(['shift','2'],shell=True)
...
if (str(GENERATE_LOG) != '' ):
    ...
    subprocess.call(['mkdir','-p',str(TMP_DIR)],shell=True)
    Make('FILE_MD5').setValue(os.popen('string_to_md5 \''+str(OUT)+'\'').read().rstrip('\n'))
    Make('TOOL_LOG_FILE').setValue(str(TMP_DIR)+'/'+str(FILE_MD5)+'.tool')
# Raw.
if (str(MODE) == 'raw' ):
    # Entry point for THUMB must be odd.
    if if str(ARCH) == 'thumb':
        (RAW_ENTRY_POINT % 2) == 0:
        Make('RAW_ENTRY_POINT').setValue((RAW_ENTRY_POINT + 1))
    Make('KEEP_UNREACHABLE_FUNCS').setValue(1)
# Check for archives.
if (str(MODE) == 'bin' ):
    ...
    if (subprocess.call(['is_macho_archive',str(IN)],shell=True) ):
        Make('OUT_ARCHIVE').setValue(str(OUT)+'.a')
        if (str(ARCH) != '' ):
            print()
            ...
            if (not subprocess.call([str(EXTRACT),'--family',str(ARCH),'--out',str(OUT_ARCHIVE),str(IN)],shell=True) ):
    ...
    if (subprocess.call(['has_archive_signature',str(IN)],shell=True) ):
        ...
        Make('ARCH_OBJECT_COUNT').setValue(os.popen('archive_object_count \''+str(IN)+'\'').read().rstrip('\n'))
        if (int(ARCH_OBJECT_COUNT) <= 0 ):
            cleanup()
            subprocess.call(['print_error_and_die','The input archive is empty.'],shell=True)
        # Prepare object output path.
        Make('OUT_RESTORED').setValue(str(OUT)+'.restored')
        ...
        if (str(AR_INDEX) != '' ):
            ...
            if (not subprocess.call(['archive_get_by_index',str(IN),str(AR_INDEX),str(OUT_RESTORED)],shell=True) ):
                cleanup()
                Make('VALID_INDEX').setValue((ARCH_OBJECT_COUNT - 1))
                if (int(VALID_INDEX) != 0 ):
                    subprocess.call(['print_error_and_die','File on index \''+str(AR_INDEX)+'\' was not found in the input archive. Valid indexes are 0-'+str(VALID_INDEX)+'.'],shell=True)
                else:
                    subprocess.call(['print_error_and_die','File on index \''+str(AR_INDEX)+'\' was not found in the input archive. The only valid index is 0.'],shell=True)
            Make('IN').setValue(OUT_RESTORED)
        ...
        str(AR_NAME) != '' ):
            ...
            if (not subprocess.call(['archive_get_by_name',str(IN),str(AR_NAME),str(OUT_RESTORED)],shell=True) ):
                cleanup()
                subprocess.call(['print_error_and_die','File named \''+str(AR_NAME)+'\' was not found in the input archive.'],shell=True)
            Make('IN').setValue(OUT_RESTORED)
        else:
            # Print list of files.
            print('Please select file to decompile with either '--ar-index=n'')
            print('or '--ar-name=string' option. Archive contains these files:')
            subprocess.call(['archive_list_numbered_content',str(IN)],shell=True)
            cleanup()
            exit(1)
    else:
        if str(AR_NAME) != '':
...
if (if not str(MODE) == 'bin':
    str(MODE) == 'raw' ):
    # Assignment of other used variables.
    Make('OUT_UNPACKED').setValue(str(OUT%.*)+'-unpacked')
    Make('OUT_FRONTEND').setValue(str(OUT)+'.frontend')
    Make('OUT_FRONTEND_LL').setValue(str(OUT_FRONTEND)+'.ll')
    Make('OUT_FRONTEND_BC').setValue(str(OUT_FRONTEND)+'.bc')
    Make('CONFIG').setValue(str(OUT)+'.json')
    if (str(CONFIG) != str(CONFIG_DB) ):
        subprocess.call(['rm','-f',str(CONFIG)],shell=True)
    if (str(CONFIG_DB) != '' ):
    ...
    ##
    Make('FILEINFO_PARAMS').setValue('(-c '+str(CONFIG)+' --similarity '+str(IN)+' --no-hashes=all)')
    if (str(FILEINFO_VERBOSE) != '' ):
        Make('FILEINFO_PARAMS').setValue('(-c '+str(CONFIG)+' --similarity --verbose '+str(IN)+')')
    for Make('par') in Array(FILEINFO_EXTERNAL_YARA_PRIMARY_CRYPTO_DATABASES[@] ]):
        Make('FILEINFO_PARAMS').setValue('(--crypto '+str(par)+')')
    if (str(FILEINFO_USE_ALL_EXTERNAL_PATTERNS) != '' ):
        for Make('par') in Array(FILEINFO_EXTERNAL_YARA_EXTRA_CRYPTO_DATABASES[@] ]):
            Make('FILEINFO_PARAMS').setValue('(--crypto '+str(par)+')')
    if (not str(MAX_MEMORY) == '' ):
        Make('FILEINFO_PARAMS').setValue('(--max-memory '+str(MAX_MEMORY)+')')
    ...
    if (str(GENERATE_LOG) != '' ):
        Make('FILEINFO_AND_TIME_OUTPUT').setValue(os.popen(str(TIME)+' \''+str(FILEINFO)+'\' \''+str(FILEINFO_PARAMS[@] ])+'\' 2>&1').read().rstrip('\n'))
        Make('FILEINFO_RC').setValue(_rc0)
        Make('LOG_FILEINFO_RC').setValue(os.popen('get_tool_rc \''+str(FILEINFO_RC)+'\' \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
        Make('LOG_FILEINFO_RUNTIME').setValue(os.popen('get_tool_runtime \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
        Make('LOG_FILEINFO_MEMORY').setValue(os.popen('get_tool_memory_usage \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
        Make('LOG_FILEINFO_OUTPUT').setValue(os.popen('get_tool_output \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
    ...
    ## Unpacking.
    ##
    Make('UNPACK_PARAMS').setValue('(--extended-exit-codes --output '+str(OUT_UNPACKED)+' '+str(IN)+')')
    if (not str(MAX_MEMORY) == '' ):
        Make('UNPACK_PARAMS').setValue('(--max-memory '+str(MAX_MEMORY)+')')
    elif (str(NO_MEMORY_LIMIT) == '' ):
    ...
    if (str(GENERATE_LOG) != '' ):
        Make('LOG_UNPACKER_OUTPUT').setValue(os.popen(str(UNPACK_SH)+' \''+str(UNPACK_PARAMS[@] ])+'\' 2>&1').read().rstrip('\n'))
        Make('UNPACKER_RC').setValue(_rc0)
        Make('LOG_UNPACKER_RC').setValue(UNPACKER_RC)
    else:
    ...
    if (if not if not int(UNPACKER_RC) == 0:
        ...
        if (str(GENERATE_LOG) != '' ):
            ...
            Make('FILEINFO_RC').setValue(_rc0)
            Make('LOG_FILEINFO_RC').setValue(os.popen('get_tool_rc \''+str(FILEINFO_RC)+'\' \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
            Make('FILEINFO_RUNTIME').setValue(os.popen('get_tool_runtime \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
            Make('LOG_FILEINFO_RUNTIME').setValue((LOG_FILEINFO_RUNTIME + FILEINFO_RUNTIME))
            Make('FILEINFO_MEMORY').setValue(os.popen('get_tool_memory_usage \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
            Make('LOG_FILEINFO_MEMORY').setValue(((LOG_FILEINFO_MEMORY + FILEINFO_MEMORY) // 2))
            Make('LOG_FILEINFO_OUTPUT').setValue(os.popen('get_tool_output \''+str(FILEINFO_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
            print(LOG_FILEINFO_OUTPUT)
        else:
            subprocess.call([str(FILEINFO),str(FILEINFO_PARAMS[@] ])],shell=True)
    ...
    if (str(ARCH) != '' ):
        ...
        Make('ARCH_FULL').setValue(os.popen('\''+str(CONFIGTOOL)+'\' \''+str(CONFIG)+'\' --read --arch | awk \'{print tolower($0').read().rstrip('\n')+'})')
        # Strip comments in parentheses and all trailing whitespace
        Make('ARCH').setValue(os.popen('echo '+str(ARCH_FULL%(*)+' | sed -e 's/^[[:space:]]*//'').read().rstrip('\n'))
    # Get object file format.
    Make('FORMAT').setValue(os.popen('\''+str(CONFIGTOOL)+'\' \''+str(CONFIG)+'\' --read --format | awk \'{print tolower($1').read().rstrip('\n')+';})')
    ...
    if (str(ARCH) == 'arm' -o str(ARCH)'=' != '' ):
        Make('ORDS_DIR').setValue(ARM_ORDS_DIR)
    elif (str(ARCH) == 'x86' ):
        Make('ORDS_DIR').setValue(X86_ORDS_DIR)
    elif (str(ARCH) == 'powerpc' -o str(ARCH) '='  '-o' str(ARCH) == 'pic32' ):
        pass
    else:
        # nothing
        if str(GENERATE_LOG) != '':
            generate_log()
        cleanup()
        subprocess.call(['print_error_and_die','Unsupported target architecture ''+str(ARCH^^)+''. Supported architectures: Intel x86, ARM, ARM+Thumb, MIPS, PIC32, PowerPC.'],shell=True)
    ...
    Make('FILECLASS').setValue(os.popen('\''+str(CONFIGTOOL)+'\' \''+str(CONFIG)+'\' --read --file-class').read().rstrip('\n'))
    if (if str(FILECLASS) != '16':
        str(FILECLASS) != '32' ):
        if str(GENERATE_LOG) != '':
            generate_log()
        cleanup()
        subprocess.call(['print_error_and_die','Unsupported target format ''+str(FORMAT^^)+str(FILECLASS)+''. Supported formats: ELF32, PE32, Intel HEX 32, Mach-O 32.'],shell=True)
    ...
    Make('SIG_FORMAT').setValue(FORMAT,,)
    if (if not str(SIG_FORMAT) == 'ihex':
        str(SIG_FORMAT) == 'raw' ):
        Make('SIG_FORMAT').setValue('elf')
    Make('ENDIAN').setValue(os.popen('\''+str(CONFIGTOOL)+'\' \''+str(CONFIG)+'\' --read --endian').read().rstrip('\n'))
    ...
    Make('SIG_ARCH').setValue(ARCH,,)
    if (str(SIG_ARCH) == 'pic32' ):
        Make('SIG_ARCH').setValue('mips')
    Make('SIGNATURES_DIR').setValue(str(GENERIC_SIGNATURES_DIR)+'/'+str(SIG_FORMAT)+'/'+str(FILECLASS,,)+'/'+str(SIG_ENDIAN,,)+'/'+str(SIG_ARCH))
    print_warning_if_decompiling_bytecode()
    # Decompile unreachable functions.
    if (str(KEEP_UNREACHABLE_FUNCS) != '' ):
        subprocess.call([str(CONFIGTOOL),str(CONFIG),'--write','--keep-unreachable-funcs','true'],shell=True)
    ...
    while (l < Expand.hash()SIGNATURE_ARCHIVE_PATHS[@]):
        Make('LIB').setValue(SIGNATURE_ARCHIVE_PATHS[l] ])
        print('Extracting signatures from file ''+str(LIB)+''')
        Make('CROP_ARCH_PATH').setValue(os.popen('basename \''+str(LIB)+'\' | LC_ALL=C sed -e \'s/[^A-Za-z0-9_.-]/_/g\'').read().rstrip('\n'))
        Make('SIG_OUT').setValue(str(OUT)+'.'+str(CROP_ARCH_PATH)+'.'+str(l)+'.yara')
        if (subprocess.call(str(SIG_FROM_LIB_SH) + ' ' + str(LIB) + ' ' + '--output' + ' ' + str(SIG_OUT),shell=True,stderr=subprocess.STDOUT,stdout=file(str(DEV_NULL),'wb')) ):
            subprocess.call([str(CONFIGTOOL),str(CONFIG),'--write','--user-signature',str(SIG_OUT)],shell=True)
            Make('SIGNATURES_TO_REMOVE').setValue('('+str(SIG_OUT)+')')
        else:
    ...
    subprocess.call([str(CONFIGTOOL),str(CONFIG),'--write','--frontend-output-file',str(OUT_FRONTEND_LL)],shell=True)
    ...
    if (str(SELECTED_FUNCTIONS) != '' ):
        for Make('f') in Array(SELECTED_FUNCTIONS[@] ]):
    ...
    Make('OUT_BACKEND').setValue(str(OUT)+'.backend')
    ...
    if str(OUT_FRONTEND_LL) == str(OUT_BACKEND)+'.ll':
        Make('OUT_BACKEND').setValue(str(OUT)+'.backend.backend')
    Make('OUT_BACKEND_BC').setValue(str(OUT_BACKEND)+'.bc')
    Make('OUT_BACKEND_LL').setValue(str(OUT_BACKEND)+'.ll')
    ##
    ## Decompile the binary into LLVM IR.
    ##
    if (str(KEEP_UNREACHABLE_FUNCS) != '' ):
        # Prevent bin2llvmir from removing unreachable functions.
        Make('BIN2LLVMIR_PARAMS').setValue(os.popen('sed 's/-unreachable-funcs *//g' <<< \''+str(BIN2LLVMIR_PARAMS)+'\'').read().rstrip('\n'))
    if (if str(CONFIG) == str():
        str(CONFIG_DB) != str() ):
        Make('CONFIG').setValue(CONFIG_DB)
    Make('BIN2LLVMIR_PARAMS').setValue('(-provider-init -config-path '+str(CONFIG)+' -decoder '+str(BIN2LLVMIR_PARAMS)+')')
    if (not str(MAX_MEMORY) == '' ):
        Make('BIN2LLVMIR_PARAMS').setValue('(-max-memory '+str(MAX_MEMORY)+')')
    elif (str(NO_MEMORY_LIMIT) == '' ):
    ...
    if (str(GENERATE_LOG) != '' ):
        def thread1():
            subprocess.call(str(TIME) + ' ' + str(BIN2LLVMIR) + ' ' + str(BIN2LLVMIR_PARAMS[@] ]) + ' ' + '-o' + ' ' + str(OUT_BACKEND_BC),shell=True,stdout=file(str(TOOL_LOG_FILE),'wb'),stderr=subprocess.STDOUT)
        ...
        Make('BIN2LLVMIR_AND_TIME_OUTPUT').setValue(os.popen('cat \''+str(TOOL_LOG_FILE)+'\'').read().rstrip('\n'))
        Make('LOG_BIN2LLVMIR_RC').setValue(os.popen('get_tool_rc \''+str(BIN2LLVMIR_RC)+'\' \''+str(BIN2LLVMIR_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
        Make('LOG_BIN2LLVMIR_RUNTIME').setValue(os.popen('get_tool_runtime \''+str(BIN2LLVMIR_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
        Make('LOG_BIN2LLVMIR_MEMORY').setValue(os.popen('get_tool_memory_usage \''+str(BIN2LLVMIR_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
        Make('LOG_BIN2LLVMIR_OUTPUT').setValue(os.popen('get_tool_output \''+str(BIN2LLVMIR_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
...
if (str(MODE) == 'll' ):
    Make('OUT_BACKEND_BC').setValue(IN)
    Make('CONFIG').setValue(CONFIG_DB)
# Conditional initialization.
...
LLVMIR2HLL_PARAMS=Bash2Py('(-target-hll='+str(HLL)+' -var-renamer='+str(BACKEND_VAR_RENAMER)+' -var-name-gen=fruit -var-name-gen-prefix= -call-info-obtainer='+str(BACKEND_CALL_INFO_OBTAINER)+' -arithm-expr-evaluator='+str(BACKEND_ARITHM_EXPR_EVALUATOR)+' -validate-module -llvmir2bir-converter='+str(BACKEND_LLVMIR2BIR_CONVERTER)+' -o '+str(OUT)+' '+str(OUT_BACKEND_BC)+')')
if str(BACKEND_NO_DEBUG) == '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-enable-debug)')
if str(BACKEND_NO_DEBUG_COMMENTS) == '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-emit-debug-comments)')
if str(CONFIG) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-config-path='+str(CONFIG)+')')
if str(KEEP_UNREACHABLE_FUNCS) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-keep-unreachable-funcs)')
if str(BACKEND_SEMANTICS) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-semantics '+str(BACKEND_SEMANTICS)+')')
if str(BACKEND_ENABLED_OPTS) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-enabled-opts='+str(BACKEND_ENABLED_OPTS)+')')
if str(BACKEND_DISABLED_OPTS) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-disabled-opts='+str(BACKEND_DISABLED_OPTS)+')')
if str(BACKEND_OPTS_DISABLED) != '':
...
if str(BACKEND_FIND_PATTERNS) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-find-patterns '+str(BACKEND_FIND_PATTERNS)+')')
if str(BACKEND_EMIT_CG) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-emit-cg)')
if str(BACKEND_FORCED_MODULE_NAME) != '':
    Make('LLVMIR2HLL_PARAMS').setValue('(-force-module-name='+str(BACKEND_FORCED_MODULE_NAME)+')')
if str(BACKEND_STRICT_FPU_SEMANTICS) != '':
...
if (not str(MAX_MEMORY) == '' ):
    Make('LLVMIR2HLL_PARAMS').setValue('(-max-memory '+str(MAX_MEMORY)+')')
elif (str(NO_MEMORY_LIMIT) == '' ):
...
if (str(GENERATE_LOG) != '' ):
    def thread3():
        subprocess.call(str(TIME) + ' ' + str(LLVMIR2HLL) + ' ' + str(LLVMIR2HLL_PARAMS[@] ]),shell=True,stdout=file(str(TOOL_LOG_FILE),'wb'),stderr=subprocess.STDOUT)
    ...
    Make('LLVMIR2HLL_AND_TIME_OUTPUT').setValue(os.popen('cat \''+str(TOOL_LOG_FILE)+'\'').read().rstrip('\n'))
    Make('LOG_LLVMIR2HLL_RC').setValue(os.popen('get_tool_rc \''+str(LLVMIR2HLL_RC)+'\' \''+str(LLVMIR2HLL_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
    Make('LOG_LLVMIR2HLL_RUNTIME').setValue(os.popen('get_tool_runtime \''+str(LLVMIR2HLL_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
    Make('LOG_LLVMIR2HLL_MEMORY').setValue(os.popen('get_tool_memory_usage \''+str(LLVMIR2HLL_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
    Make('LOG_LLVMIR2HLL_OUTPUT').setValue(os.popen('get_tool_output \''+str(LLVMIR2HLL_AND_TIME_OUTPUT)+'\'').read().rstrip('\n'))
...
if (( str(BACKEND_EMIT_CG) != '' and str(BACKEND_CG_CONVERSION) == 'auto' ) or ( str(BACKEND_EMIT_CFG) != '' and str(BACKEND_CFG_CONVERSION) == 'auto' ) ):
    print()
...
if (if str(BACKEND_EMIT_CG) != '':
    ...
    subprocess.call('dot' + ' ' + '-T'+str(GRAPH_FORMAT) + ' ' + str(OUT)+'.cg.dot',shell=True,stdout=file(str(OUT)+'.cg.'+str(GRAPH_FORMAT),'wb'))

if (if str(BACKEND_EMIT_CFG) != '':
    str(BACKEND_CFG_CONVERSION) == 'auto' ):
    for Make('cfg') in Glob(str(OUT)+'.cfg.*.dot'):
        print('RUN: dot -T'+str(GRAPH_FORMAT)+' '+str(cfg)+' > '+str(cfg%.*)+'.'+str(GRAPH_FORMAT))
        subprocess.call('dot' + ' ' + '-T'+str(GRAPH_FORMAT) + ' ' + str(cfg),shell=True,stdout=file(str(cfg%.*)+'.'+str(GRAPH_FORMAT),'wb'))
...
_rc4 = subprocess.call('sed' + ' ' + '-e' + ' ' + ':a' + ' ' + '-e' + ' ' + '/^\\n*$/{$d;N;};/\\n$/ba' + ' ' + '-e' + ' ' + 's/[[:space:]]*$//',shell=True,stdin=file(str(OUT),'rb'),stdout=file(str(OUT)+'.tmp','wb'))
